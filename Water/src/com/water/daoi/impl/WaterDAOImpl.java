...
public class WaterDAOImpl extends BaseHibernateDAO implements WaterDAOI {
	...
	public boolean updateWaterCostDetails(WaterBusiness waterBusiness) {
		log.debug("attaching dirty WaterSupply instance");
		try {
			System.out.println(waterBusiness.getBusinessid());
			Session session = getSession();
			Transaction transaction = session.beginTransaction();
			WaterBusiness instance = (WaterBusiness) session.get(
					WaterBusiness.class, waterBusiness.getBusinessid());
			instance.setBusinessfor(waterBusiness.getBusinessfor());
			instance.setWaterliters(waterBusiness.getWaterliters());
			instance.setWateramount(waterBusiness.getWateramount());
			instance.setWaterdesc(waterBusiness.getWaterdesc());
			session.update(instance);
			transaction.commit();
			log.debug("attach successful");
			flag = true;
